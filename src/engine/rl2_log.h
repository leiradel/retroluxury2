...
void rl2_log(rl2_LogLevel level, char const* format, ...);
    ...
    #ifdef RL2_BUILD_DEBUG
        #define RL2_DEBUG(...) do { rl2_log(RL2_LOG_DEBUG, __FILE__, __LINE__, __VA_ARGS__); } while (0)
    #else
...
#ifdef RL2_BUILD_DEBUG
    #define RL2_INFO(...) do { rl2_log(RL2_LOG_INFO, __FILE__, __LINE__, __VA_ARGS__); } while (0)
    #define RL2_WARN(...) do { rl2_log(RL2_LOG_WARN, __FILE__, __LINE__, __VA_ARGS__); } while (0)
    #define RL2_ERROR(...) do { rl2_log(RL2_LOG_ERROR, __FILE__, __LINE__, __VA_ARGS__); } while (0)
#else
